...
The same repair runs the moment a customer comes back from checkout: the return carries a marker the app reads once, and it reconciles the workspace there and then rather than waiting for a webhook that may never arrive.
