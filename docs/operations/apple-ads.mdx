...
description: Give asobeast an Apple Ads API key so App Store traffic uses Apple's own weekly search popularity where Apple publishes it. Optional and off by default.
icon: key-round
keywords: ["Apple Ads","search popularity","API key","prime256v1","APPLE_ADS_CLIENT_ID","calibration"]
---

Without a key, App Store traffic is an estimate from public store signals. With one, asobeast reads Apple's weekly search popularity for the terms Apple publishes and uses it instead. See [Traffic, difficulty and opportunity](/concepts/scoring#can-asobeast-use-apples-own-numbers) for what changes in the scores.
