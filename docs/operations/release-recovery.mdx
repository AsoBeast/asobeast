...
pnpm check:rate-classes && pnpm check:compose-limits && pnpm check:env-documented
pnpm check:image-defaults && pnpm check:release-versions && pnpm docs:mcp-tools:check
```

Then bump the version everywhere Release Please would have. Eight files move together, and `.release-please-manifest.json` must agree with them or `verify-release-state.sh` fails the next Release run. `pnpm check:release-versions` reports any that were missed:
