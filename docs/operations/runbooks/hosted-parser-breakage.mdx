...
1. **Tell customers within the hour.** Post the incident, and say which store is affected and that stored data is untouched.
2. Pause that store's queue. Failed jobs retry with backoff and add load to a pool that cannot use it.
3. Leave the other store running. Half a product is much better than none, and the stores fail independently by design.
4. Do not roll back the database. Collection pauses; it never rewrites history.
