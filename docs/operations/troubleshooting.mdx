...
<AccordionGroup>
  ...
  <Accordion title="Scores changed after upgrading">
    Expected after an upgrade that changes the scoring formula. Each score row records the `formulaVersion` that produced it, and the API queues one rescore for every tracked keyword whose latest score used an older version, through the same rate limited queues as weekly scoring. Until its job runs, a keyword keeps its old numbers and the table says "Older formula". A large list takes a while: about 100 minutes per 150 App Store keywords at the default rate limit. See [Traffic, difficulty and opportunity](/concepts/scoring#what-happens-to-old-scores-after-an-upgrade).
  </Accordion>
