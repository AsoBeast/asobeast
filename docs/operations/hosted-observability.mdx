...
| `billing.webhooks.failing` | A stored billing event recorded a failure and still waits for a fix | [Billing incident](/operations/runbooks/billing-incident) |
...
| `billing.orphan.events` | A billing event in the last seven days named a subscription this instance cannot place |
| `billing.orphan.subscriptions` | The last nightly reconciliation found Stripe subscriptions that belong to no workspace. The summary lists up to five ids |
...
| `POST /admin/support/workspaces/{id}/billing-events/{eventId}/replay` | Clears how a stored billing event settled and queues it again, once its cause is fixed. Answers 404 for an event that names neither the workspace nor its Stripe customer |
