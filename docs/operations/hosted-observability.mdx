...
`ERROR_TRACKING_DSN` sends server errors to a Sentry compatible endpoint. It is ignored unless `BILLING_ENABLED` is `true`, so **a self hosted deployment never reports its errors to anyone**, and the end to end suite asserts that a deployment with no cloud configuration opens no connection beyond its own database and queue.

A report carries the error type, a secret scrubbed message and stack, the workspace and correlation id, the request method, and the request path with identifiers replaced by `:id`. It never carries headers, cookies, query strings, request bodies or account details.
