...
The API reports through the official Sentry SDK, behind a gate that stays closed unless all three hold: `ERROR_TRACKING_DSN` is set, `NODE_ENV` is `production` and `BILLING_ENABLED` is `true`.

When the gate is closed the SDK is never initialized, so nothing is instrumented and nothing is sent. **A self hosted deployment never reports its errors to anyone**, and the end to end suite asserts that a deployment with no cloud configuration opens no connection beyond its own database and queue.

### What is reported

| Surface | Covers |
| --- | --- |
| Requests | Only `5xx`. A refusal the API meant to make, such as a quota or a rate limit, is not an incident |
| Queue work | A job whose retry chain is exhausted, once, tagged with the job, the queue and the store. The job payload is never attached |
| The process | An uncaught exception or an unhandled rejection outside any request or job |
...
Breadcrumbs are turned off. A breadcrumb records the url of every outgoing request, and a store request carries the term a customer tracks in its query string, so the trail that would help debugging is the one place customer data would reach a report.
