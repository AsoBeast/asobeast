...
The first pass reuses the identifiers the on demand runs already build, so a repeat import collects nothing twice.

| Work | Identifier | What deduplicates against it |
| --- | --- | --- |
| Rank check | `check~<keywordId>~<date>` | A repeat import, and **Run daily** on the same app the same UTC day |
...
The nightly pipeline is deliberately not on that list. It builds its own `daily~` prefixed identifiers so its flow keeps track of its own children, and it regenerates actions in place rather than queueing a job. So an import in the hours before `CRON_DAILY` fires can have a keyword checked twice that day. The second check overwrites the same stored position rather than adding a row, so the cost is one store request per keyword and nothing else.
