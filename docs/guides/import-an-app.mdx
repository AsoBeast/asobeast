...
- **Positions are empty right after importing.** Expected for a moment. The rank checks are queued at import and drain at the store rate limit, so give the queue a few minutes. If they stay empty, check the queue dashboard at `/admin/queues` for failed jobs.
