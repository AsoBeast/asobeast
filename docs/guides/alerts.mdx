...
A keyword that moves between 10 and 11 fires each time it crosses. Every fact is deduplicated per keyword and day and folded into the batched report, so a noisy boundary costs a line in the daily report rather than a message per check.
