...
Four things the schema cannot tell you.
...
- **The last save wins.** Saves to one app run one at a time, so two saves that overlap never merge: the field holds whichever finished last. Neither is refused for overlapping, and because the body describes the stored field, a save that overlaps another may be answered with the other save's field.
