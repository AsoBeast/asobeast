...
Because the browser and the API share an origin, there is no CORS configuration and no cross site cookie handling. `API_PROXY_TIMEOUT_MS` bounds how long each proxied request waits for the API to start answering, and a request that exceeds it returns a `504` error envelope instead of hanging.
