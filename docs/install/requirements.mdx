...
| --- | --- | --- |
| Node.js | 22.12 or newer | The images build on `node:24-alpine` |
| pnpm | 12.4.1 | `packageManager` in the root `package.json` |
...
<Warning>
  Publishing host port 3001 to the internet without TLS in front of it sends session cookies in the clear. Terminate TLS at a reverse proxy. `AUTH_COOKIE_SECURE` is already `true` in the Compose stack and must stay so, because the Docker image refuses to boot without it.
</Warning>
