...
**It is empty by default, and while it is empty no request is ever made.** The poll is not even scheduled. `apps/api/test/no-outbound.e2e-spec.ts` asserts exactly that, and it is one of five opt in outbound integrations alongside webhook alerts, email, OpenAI assistance and Apple Ads search popularity. The last one is a weekly authenticated read of Apple's published search popularity, made only when the operator configures their own Apple Ads API key. See [Connect Apple Ads](/operations/apple-ads).
