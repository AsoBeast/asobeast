...
The keyword field text may run to ten times the 100 characters Apple indexes, so the API does not refuse a pasted field for its spacing or duplicates. What asobeast stores is held to Apple's limit: once each phrase is normalized and the duplicates are removed, a field longer than 100 characters returns `400` and writes nothing. See [App Store and Google Play](/concepts/stores).
