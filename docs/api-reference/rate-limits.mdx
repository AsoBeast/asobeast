...
Two ceilings are resource safety rather than plan capacity, so they hold on a self hosted instance too.
...
The keyword field text may run to ten times the 100 characters Apple indexes, so spacing, casing and duplicates in a pasted field never cost a refusal. What asobeast stores is held to Apple's limit: once each phrase is normalized and the duplicates are removed, a field longer than 100 characters returns `400` and writes nothing. See [App Store and Google Play](/concepts/stores).
