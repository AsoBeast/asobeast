...
- Give a window. `last 30 days` maps directly onto the `from` and `to` parameters, which must be real dates in order. See [API conventions](/api-reference/conventions#dates-are-utc).
