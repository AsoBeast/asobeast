...
5. **Opportunity is per app, not per keyword.** Traffic and difficulty persist in `KeywordMetric`; opportunity depends on the app's keyword relevance and is computed in the read layer only: volume × a squared difficulty gate × relevance, with the difficulty shifted by the app's rating count against the page median (`scoring/opportunity.ts`, `scoring/keyword-opportunity.ts`).
