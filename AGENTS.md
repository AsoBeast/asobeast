...
- Dependabot pull requests whose highest change is patch or minor merge on their own through `.github/workflows/dependabot-auto-merge.yml`, gated by the required checks on `main`. Majors travel in their own group and wait for a person. `pmOnFail: ignore` in `pnpm-workspace.yaml` keeps `pnpm-lock.yaml` a single YAML document, which Dependabot and the dependency graph need to read it; pnpm 12 would otherwise prepend a document recording its own version.
